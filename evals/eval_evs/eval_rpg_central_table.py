"""Tableau central DEVO sur RPG (Vanilla / M1 / M2 / M3).

Pas de M0 oracle : RPG ne contient pas d'objets dynamiques annotés.
M1/M2/M3 appliquent le modèle MS en inférence pure ; le modèle devrait donner
des scores proches de 0 (pas d'objets dynamiques), laissant DEVO quasi-vanilla.

Usage :
    conda activate devofou && cd DEVO
    python evals/eval_evs/eval_rpg_central_table.py \
        --datapath /path/to/rpg_dataset \
        --weights DEVO.pth \
        --ms_config /path/to/MS_Model/configs/convlstm_v4_full.yaml \
        [--ms_decoupled /path/ms_final_m1.pt] \
        [--ms_coupled_sup /path/ms_final_m2.pt] \
        [--ms_coupled_selfsup /path/ms_final_m3.pt] \
        [--devo_coupled_sup /path/devo_m2.pth] \
        [--devo_coupled_selfsup /path/devo_m3.pth] \
        --outdir results/rpg_central_table
"""
import os
import csv
import argparse
...
from eval_rpg_evs import evaluate
...
if __name__ == "__main__":
    ...
    p.add_argument('--config', default="config/eval_rpg.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/rpg/rpg_val.txt")
    ...
    p.add_argument('--outdir', default="results/rpg_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("rpg", weights, args.ms_config,
                               side=args.side, threshold=args.threshold)

    rows = []
    rows.append(("DEVO vanilla", run_row("DEVO vanilla", cfg, args, args.weights, None, common)[0]))

    if args.ms_decoupled:
        rows.append(("DEVO + M1 (découplé)", run_row("DEVO + M1 (découplé)", cfg, args, args.weights, ms_factory(args.ms_decoupled), common)[0]))
    else:
        rows.append(("DEVO + M1 (découplé)", None))

    if args.ms_coupled_sup:
        rows.append(("DEVO + M2 (couplé sup)", run_row("DEVO + M2 (couplé sup)", cfg, args,
                     args.devo_coupled_sup or args.weights, ms_factory(args.ms_coupled_sup), common)[0]))
    else:
        rows.append(("DEVO + M2 (couplé sup)", None))

    if args.ms_coupled_selfsup:
        rows.append(("DEVO + M3 (couplé auto-sup)", run_row("DEVO + M3 (couplé auto-sup)", cfg, args,
                     args.devo_coupled_selfsup or args.weights, ms_factory(args.ms_coupled_selfsup), common)[0]))
    else:
        rows.append(("DEVO + M3 (couplé auto-sup)", None))
