"""Tableau central DEVO sur TUM-VIE (Vanilla / M1 / M2 / M3).
Note : résolution 720×1280, différente du training EVIMO (260×346).
Le ConvLSTM est fully-conv donc fonctionne, mais les features peuvent différer.
"""
import os
import csv
import argparse
...
from eval_tumvie_evs import evaluate
...
def write_table(rows, outdir):
    ...
    print(f"[tumvie-table] → {outdir}/central_table.md")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--config', default="config/eval_tumvie.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/tumvie/tumvie_val.txt")
    ...
    p.add_argument('--camID', type=int, default=2)
    p.add_argument('--threshold', type=float, default=None)
    p.add_argument('--trials', type=int, default=1)
    p.add_argument('--stride', type=int, default=1)
    p.add_argument('--outdir', default="results/tumvie_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("tumvie", weights, args.ms_config,
                               camID=args.camID, threshold=args.threshold)
