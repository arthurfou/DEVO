"""Tableau central DEVO sur FPV (Vanilla / M1 / M2 / M3).
Seules les séquences _with_gt contribuent aux métriques.
"""
import os
import csv
import argparse
...
from eval_fpv_evs import evaluate
...
def write_table(rows, outdir):
    ...
    print(f"[fpv-table] → {outdir}/central_table.md")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--config', default="config/eval_fpv.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/fpv/fpv_val.txt")
    ...
    p.add_argument('--outdir', default="results/fpv_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("fpv", weights, args.ms_config, threshold=args.threshold)
