"""Tableau central DEVO sur MVSEC (Vanilla / M1 / M2 / M3)."""
import os
import csv
import argparse
...
from eval_mvsec_evs import evaluate
...
def write_table(rows, outdir):
    ...
    print(f"[mvsec-table] → {outdir}/central_table.md")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--config', default="config/eval_mvsec.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/mvsec/mvsec_val.txt")
    ...
    p.add_argument('--outdir', default="results/mvsec_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("mvsec", weights, args.ms_config,
