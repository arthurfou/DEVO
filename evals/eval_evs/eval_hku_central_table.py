"""Tableau central DEVO sur HKU (Vanilla / M1 / M2 / M3)."""
import os
import csv
import argparse
...
from eval_hku_evs import evaluate
...
def write_table(rows, outdir):
    ...
    print(f"[hku-table] → {outdir}/central_table.md")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--config', default="config/eval_hku.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/hku/hku_val.txt")
    ...
    p.add_argument('--outdir', default="results/hku_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("hku", weights, args.ms_config,
