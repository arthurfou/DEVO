"""Tableau central DEVO sur EDS (Vanilla / M1 / M2 / M3)."""
import os
import csv
import argparse
...
from eval_eds_evs import evaluate
...
def write_table(rows, outdir):
    ...
    print(f"[eds-table] → {outdir}/central_table.md")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--config', default="config/eval_eds.yaml")
    p.add_argument('--datapath', required=True)
    p.add_argument('--val_split', default="splits/eds/eds_val.txt")
    ...
    p.add_argument('--calib1', action="store_true")
    p.add_argument('--threshold', type=float, default=None)
    p.add_argument('--trials', type=int, default=1)
    p.add_argument('--stride', type=int, default=1)
    p.add_argument('--outdir', default="results/eds_central_table")
    ...
    def ms_factory(weights):
        return make_ms_factory("eds", weights, args.ms_config, threshold=args.threshold)
